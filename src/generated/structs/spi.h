...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 38;     /*!< spi's identifier. */
     ...
     * data. 1000 9-bit data. 1001 10-bit data. 1010 11-bit data. 1011 12-bit
     * data. 1100 13-bit data. 1101 14-bit data. 1110 15-bit data. 1111 16-bit
     * data.
     */
     ...
     * transmit serial shifter is connected to input of receive serial shifter
     * internally.
     */
     ...
     * that is less than 16 bits. Unused bits at the top are ignored by
     ...
     * transmit logic. The receive logic automatically right-justifies.
     */
    inline void set_SSPDR(uint16_t value) volatile
    {
        uint32_t curr = SSPDR;
     ...
     * Get SSPSR's TFE bit.
     *
     * Transmit FIFO empty, RO: 0 Transmit FIFO is not empty. 1 Transmit FIFO
     * is empty.
     */
     ...
     * Get SSPSR's RFF bit.
     *
     * Receive FIFO full, RO: 0 Receive FIFO is not full. 1 Receive FIFO is
     * full.
     */
     ...
     * the frequency of SSPCLK. The least significant bit always returns zero
     * on reads.
     */
     ...
     * on reads.
     */
    inline void set_SSPCPSR(uint8_t value) volatile
    {
     ...
     * interrupt is masked. 1 Receive FIFO half full or less condition
     ...
     *
     * Gives the receive over run masked interrupt status, after masking, of
     ...
     *
     * (read-only) Peripheral identification registers, SSPPeriphID0-3 on page
     * 3-13
     */
...
inline volatile spi *spi_instance(void)
    requires(instance == 0)
{
    return SPI0;
}
