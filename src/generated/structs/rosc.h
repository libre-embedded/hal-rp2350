...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 31;   /*!< rosc's identifier. */
    ...
    uint32_t FREQA;   /*!< (read-write) The FREQA & FREQB registers control the
           frequency by controlling the drive strength of each stage   The drive
           strength has 4 levels determined by the number of bits set   Increasing
           the number of bits set increases the drive strength and increases the
           oscillation frequency   0 bits set is the default drive strength   1 bit
           set doubles the drive strength   2 bits set triples drive strength   3
           bits set quadruples drive strength   For frequency randomisation set
           both DS0_RANDOM=1 & DS1_RANDOM=1 */
    uint32_t FREQB;   /*!< (read-write) For a detailed description see freqa
                         register */
    uint32_t RANDOM;  /*!< (read-write) Loads a value to the LFSR randomiser */
    ...
    uint32_t COUNT; /*!< (read-write) A down counter running at the ROSC
         frequency which counts to zero and stops. To start the counter write a
         non-zero value. Can be used for short software pauses when setting up
     ...
     *                             TOOHIGH uses stages 0 to 1 and should not be
     * used because its frequency exceeds design specifications The clock
     * output will not glitch when changing the range up one step at a time The
     * clock output will glitch when changing the range down Note: the values
     * here are gray coded which is why HIGH comes before TOOHIGH
     */
    inline ROSC_CTRL_FREQ_RANGE get_CTRL_FREQ_RANGE() volatile
    {
        return ROSC_CTRL_FREQ_RANGE(CTRL & 0xfffu);
    }
     ...
     *                             The system clock must be switched to another
     * source before setting this field to DISABLE otherwise the chip will lock
     * up The 12-bit code is intended to give some protection against
     * accidental writes. An invalid setting will enable the oscillator.
     */
    inline ROSC_CTRL_ENABLE get_CTRL_ENABLE() volatile
    {
        return ROSC_CTRL_ENABLE((CTRL >> 12u) & 0xfffu);
    }
                         ...
                         ROSC_CTRL_ENABLE &ENABLE) volatile
    {
        ...
        FREQ_RANGE = ROSC_CTRL_FREQ_RANGE(curr & 0xfffu);
        ENABLE = ROSC_CTRL_ENABLE((curr >> 12u) & 0xfffu);
    }
    ...
    /**
     * Get FREQA's DS0_RANDOM bit.
     *
     * Randomises the stage 0 drive strength
     */
    inline bool get_FREQA_DS0_RANDOM() volatile
    {
        return FREQA & (1u << 3u);
    }

    /**
     * Set FREQA's DS0_RANDOM bit.
     *
     * Randomises the stage 0 drive strength
     */
    inline void set_FREQA_DS0_RANDOM() volatile
    {
        FREQA |= 1u << 3u;
    }

    /**
     * Clear FREQA's DS0_RANDOM bit.
     *
     * Randomises the stage 0 drive strength
     */
    inline void clear_FREQA_DS0_RANDOM() volatile
    {
        FREQA &= ~(1u << 3u);
    }

    /**
     * Toggle FREQA's DS0_RANDOM bit.
     *
     * Randomises the stage 0 drive strength
     */
    inline void toggle_FREQA_DS0_RANDOM() volatile
    {
        FREQA ^= 1u << 3u;
    }
    ...
    /**
     * Get FREQA's DS1_RANDOM bit.
     *
     * Randomises the stage 1 drive strength
     */
    inline bool get_FREQA_DS1_RANDOM() volatile
    {
        return FREQA & (1u << 7u);
    }

    /**
     * Set FREQA's DS1_RANDOM bit.
     *
     * Randomises the stage 1 drive strength
     */
    inline void set_FREQA_DS1_RANDOM() volatile
    {
        FREQA |= 1u << 7u;
    }

    /**
     * Clear FREQA's DS1_RANDOM bit.
     *
     * Randomises the stage 1 drive strength
     */
    inline void clear_FREQA_DS1_RANDOM() volatile
    {
        FREQA &= ~(1u << 7u);
    }

    /**
     * Toggle FREQA's DS1_RANDOM bit.
     *
     * Randomises the stage 1 drive strength
     */
    inline void toggle_FREQA_DS1_RANDOM() volatile
    {
        FREQA ^= 1u << 7u;
    }
     ...
     *                             Any other value in this field will set all
     * drive strengths to 0
     */
    inline ROSC_FREQA_PASSWD get_FREQA_PASSWD() volatile
    {
        return ROSC_FREQA_PASSWD((FREQA >> 16u) & 0xffffu);
    }
    ...
    inline void set_FREQA_PASSWD(ROSC_FREQA_PASSWD value) volatile
    {
        ...
        curr &= ~(0xffffu << 16u);
        curr |= (std::to_underlying(value) & 0xffffu) << 16u;
     ...
     * controlling the drive strength of each stage The drive strength has 4
     * levels determined by the number of bits set Increasing the number of
     * bits set increases the drive strength and increases the oscillation
     * frequency 0 bits set is the default drive strength 1 bit set doubles the
     * drive strength 2 bits set triples drive strength 3 bits set quadruples
     * drive strength For frequency randomisation set both DS0_RANDOM=1 &
     * DS1_RANDOM=1
     */
    inline void get_FREQA(uint8_t &DS0, bool &DS0_RANDOM, uint8_t &DS1,
                          bool &DS1_RANDOM, uint8_t &DS2, uint8_t &DS3,
                          ROSC_FREQA_PASSWD &PASSWD) volatile
    {
        uint32_t curr = FREQA;

        DS0 = curr & 0b111u;
        DS0_RANDOM = curr & (1u << 3u);
        DS1 = (curr >> 4u) & 0b111u;
        DS1_RANDOM = curr & (1u << 7u);
        DS2 = (curr >> 8u) & 0b111u;
        DS3 = (curr >> 12u) & 0b111u;
        PASSWD = ROSC_FREQA_PASSWD((curr >> 16u) & 0xffffu);
    }
     ...
     * DS1_RANDOM=1
     */
    inline void set_FREQA(uint8_t DS0, bool DS0_RANDOM, uint8_t DS1,
                          bool DS1_RANDOM, uint8_t DS2, uint8_t DS3,
                          ROSC_FREQA_PASSWD PASSWD) volatile
    {
        uint32_t curr = FREQA;
        ...
        curr |= (DS0 & 0b111u);
        curr &= ~(0b1u << 3u);
        curr |= (DS0_RANDOM & 0b1u) << 3u;
        curr &= ~(0b111u << 4u);
        curr |= (DS1 & 0b111u) << 4u;
        curr &= ~(0b1u << 7u);
        curr |= (DS1_RANDOM & 0b1u) << 7u;
        curr &= ~(0b111u << 8u);
        curr |= (DS2 & 0b111u) << 8u;
        curr &= ~(0b111u << 12u);
        curr |= (DS3 & 0b111u) << 12u;
        curr &= ~(0xffffu << 16u);
        curr |= (std::to_underlying(PASSWD) & 0xffffu) << 16u;
    ...
    {
        return ROSC_FREQB_PASSWD((FREQB >> 16u) & 0xffffu);
    }
                          ...
                          uint8_t &DS7, ROSC_FREQB_PASSWD &PASSWD) volatile
    {
        ...
        DS6 = (curr >> 8u) & 0b111u;
        DS7 = (curr >> 12u) & 0b111u;
        PASSWD = ROSC_FREQB_PASSWD((curr >> 16u) & 0xffffu);
    }
     ...
     */
    inline ROSC_DIV_DIV get_DIV() volatile
    {
        return ROSC_DIV_DIV(DIV & 0xffffu);
    }
     ...
     *                             this register resets to div=32
     */
    inline void set_DIV(ROSC_DIV_DIV value) volatile
    {
        uint32_t curr = DIV;
     ...
     *
     * set to 0xaa
     *                             any other value enables the output with
     * shift=0
     */
    inline uint8_t get_PHASE_PASSWD() volatile
    {
        return (PHASE >> 4u) & 0xffu;
    }
     ...
     */
    inline bool get_RANDOMBIT() volatile
    {
        return RANDOMBIT & 1u;
    }
...
static_assert(ifgen_struct<rosc>);

static volatile rosc *const ROSC = reinterpret_cast<rosc *>(0x400e8000);
