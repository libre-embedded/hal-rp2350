...
#pragma once
#ifndef RP2350_STRUCTS_TIMER0_H
#define RP2350_STRUCTS_TIMER0_H
 ...
 * finished by clearing the alarm_enable When an alarm fires, the corresponding
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 36;   /*!< timer0's identifier. */
    static constexpr std::size_t size = 76; /*!< timer0's size in bytes. */

    /* Fields. */
    uint32_t TIMEHW; /*!< (write-only) Write to bits 63:32 of time always write
                        timelw before timehw */
    uint32_t TIMELW; /*!< (write-only) Write to bits 31:0 of time writes do not
                        get copied to time until timehw is written */
    const uint32_t TIMEHR = {}; /*!< (read-only) Read from bits 63:32 of time
                                   always read timelr before timehr */
    const uint32_t TIMELR = {}; /*!< (read-only) Read from bits 31:0 of time */
    uint32_t
        ALARM0; /*!< (read-write) Arm alarm 0, and configure the time it will
                   fire. Once armed, the alarm fires when TIMER_ALARM0 ==
                   TIMELR. The alarm will disarm itself once it fires, and can
                   be disarmed early using the ARMED status register. */
    uint32_t
        ALARM1; /*!< (read-write) Arm alarm 1, and configure the time it will
                   fire. Once armed, the alarm fires when TIMER_ALARM1 ==
                   TIMELR. The alarm will disarm itself once it fires, and can
                   be disarmed early using the ARMED status register. */
    uint32_t
        ALARM2; /*!< (read-write) Arm alarm 2, and configure the time it will
                   fire. Once armed, the alarm fires when TIMER_ALARM2 ==
                   TIMELR. The alarm will disarm itself once it fires, and can
                   be disarmed early using the ARMED status register. */
    uint32_t
        ALARM3; /*!< (read-write) Arm alarm 3, and configure the time it will
                   fire. Once armed, the alarm fires when TIMER_ALARM3 ==
                   TIMELR. The alarm will disarm itself once it fires, and can
                   be disarmed early using the ARMED status register. */
    uint32_t
        ARMED; /*!< (read-write) Indicates the armed/disarmed status of each
                  alarm. A write to the corresponding ALARMx register arms the
                  alarm. Alarms automatically disarm upon firing, but writing
                  ones here will disarm immediately without waiting to fire. */
    const uint32_t TIMERAWH = {}; /*!< (read-only) Raw read from bits 63:32 of
                                     time (no side effects) */
    const uint32_t TIMERAWL = {}; /*!< (read-only) Raw read from bits 31:0 of
                                     time (no side effects) */
    ...
    uint32_t
        LOCKED; /*!< (read-write) Set locked bit to disable write access to
                   timer Once set, cannot be cleared (without a reset) */
    uint32_t
        SOURCE; /*!< (read-write) Selects the source for the timer. Defaults to
                   the normal tick configured in the ticks block (typically
                   configured to 1 microsecond). Writing to 1 will ignore the
                   tick and count clk_sys cycles instead. */
     ...
     */
    inline void set_ARMED(uint8_t value) volatile
    {
        uint32_t curr = ARMED;
     ...
     */
    inline bool get_PAUSE() volatile
    {
        return PAUSE & 1u;
    }
     ...
     */
    inline void set_PAUSE() volatile
    {
        PAUSE |= 1u;
    }
     ...
     */
    inline void clear_PAUSE() volatile
    {
        PAUSE &= ~(1u);
    }
     ...
     */
    inline void toggle_PAUSE() volatile
    {
        PAUSE ^= 1u;
    }

    /**
     * Get LOCKED's LOCKED bit.
     */
    inline bool get_LOCKED() volatile
    {
        return LOCKED & 1u;
    }

    /**
     * Set LOCKED's LOCKED bit.
     */
    inline void set_LOCKED() volatile
    {
        LOCKED |= 1u;
    }

    /**
     * Clear LOCKED's LOCKED bit.
     */
    inline void clear_LOCKED() volatile
    {
        LOCKED &= ~(1u);
    }

    /**
     * Toggle LOCKED's LOCKED bit.
     */
    inline void toggle_LOCKED() volatile
    {
        LOCKED ^= 1u;
    }

    /**
     * Get SOURCE's CLK_SYS bit.
     */
    inline bool get_SOURCE() volatile
    {
        return SOURCE & 1u;
    }

    /**
     * Set SOURCE's CLK_SYS bit.
     */
    inline void set_SOURCE() volatile
    {
        SOURCE |= 1u;
    }

    /**
     * Clear SOURCE's CLK_SYS bit.
     */
    inline void clear_SOURCE() volatile
    {
        SOURCE &= ~(1u);
    }

    /**
     * Toggle SOURCE's CLK_SYS bit.
     */
    inline void toggle_SOURCE() volatile
    {
        SOURCE ^= 1u;
    }
...
static_assert(sizeof(timer0) == timer0::size);
static_assert(ifgen_struct<timer0>);

static volatile timer0 *const TIMER0 = reinterpret_cast<timer0 *>(0x400b0000);

static volatile timer0 *const TIMER1 = reinterpret_cast<timer0 *>(0x400b8000);
