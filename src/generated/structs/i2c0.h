...
#pragma once
#ifndef RP2350_STRUCTS_I2C0_H
#define RP2350_STRUCTS_I2C0_H
 ...
 *
 *             List of configuration constants for the Synopsys I2C hardware
 * (you may see references to these in I2C register header; these are *fixed*
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 12;    /*!< i2c0's identifier. */
...
Bits 1 and 2 are set to 1 - Bits 3 and 10 are set to 0 When the master or slave
...
cleared in the same manner as other bits in this register. If the source of the
...
DW_apb_i2c when operating as a slave transmitter. */
    uint32_t
        IC_ACK_GENERAL_CALL; /*!< (read-write) I2C ACK General Call Register
...
If IC_ENABLE[0] has been set to 0, bits 2:1 is only be valid as soon as bit 0
               is read as '0'.
...
information about the component's parameter settings. Fields shown below are
the settings for those parameters */
     ...
     * bit is written with '1' then bit 6 should also be written with a '1'.
     */
     ...
     *
     *                             This register should be programmed only with
     * a value in the range of 1 to IC_MAX_SPEED_MODE; otherwise, hardware
     * updates this register with the value of IC_MAX_SPEED_MODE.
     *
     *                             1: standard mode (100 kbit/s)
     *
     *                             2: fast mode (<=400 kbit/s) or fast mode
     * plus (<=1000Kbit/s)
     *
     ...
     *
     *                             NOTE: During a general call address, this
     * slave does not issue the STOP_DET interrupt if STOP_DET_IF_ADDRESSED =
     * 1'b1, even if the slave responds to the general call address by
     * generating ACK. The STOP_DET interrupt is generated only when the
     ...
     *
     *                     Read/Write Access: - bit 10 is read only. - bit 11
     * is read only - bit 16 is read only - bit 17 is read only - bits 18 and
     * 19 are read only.
     */
     ...
     *
     *                             If the IC_TAR and IC_SAR are the same,
     * loopback exists but the FIFOs are shared between master and slave, so
     * full loopback is not feasible. Only one direction loopback mode is
     * supported (simplex), not duplex. A master cannot transmit to itself; it
     ...
     *
     *                     This register is 12 bits wide, and bits 31:12 are
     * reserved. This register can be written to only when IC_ENABLE[0] is set
     * to 0.
     *
     *                     Note: If the software or application is aware that
     * the DW_apb_i2c is not using the TAR address for the pending commands in
     * the Tx FIFO, then it is possible to update the TAR address even while
     * the Tx FIFO has entries (IC_STATUS[2]= 0). - It is not necessary to
     * perform any write to this register if DW_apb_i2c is enabled as an I2C
     * slave only.
     */
     ...
     * register being set to 0. Writes at other times have no effect.
     *
     *                             Note: The default values cannot be any of
     * the reserved address locations: that is, 0x00 to 0x07, or 0x78 to 0x7f.
     * The correct operation of the device is not guaranteed if you program the
     * IC_SAR or IC_TAR to a reserved value. Refer to
     * <<table_I2C_firstbyte_bit_defs>> for a complete list of these reserved
     * values.
     */
     ...
     * this bit distinguishes the write and read commands. In slave-receiver
     * mode, this bit is a 'don't care' because writes to this register are not
     * required. In slave-transmitter mode, a '0' indicates that the data in
     * IC_DATA_CMD is to be transmitted.
     *
     *                             When programming this bit, you should
     * remember the following: attempting to perform a read operation after a
     * General Call command has been sent results in a TX_ABRT interrupt (bit 6
     * of the IC_RAW_INTR_STAT register), unless bit 11 (SPECIAL) in the IC_TAR
     ...
     * empty, the master immediately tries to start a new transfer by issuing a
     * START and arbitrating for the bus. - 0 - STOP is not issued after this
     * byte, regardless of whether or not the Tx FIFO is empty. If the Tx FIFO
     * is not empty, the master continues the current transfer by
     * sending/receiving data bytes according to the value of the CMD bit. If
     * the Tx FIFO is empty, the master holds the SCL line low and stalls the
     * bus until a new command is available in the Tx FIFO. Reset value: 0x0
     */
     ...
     * issued before the data is sent/received (according to the value of CMD),
     * regardless of whether or not the transfer direction is changing from the
     * previous command; if IC_RESTART_EN is 0, a STOP followed by a START is
     * issued instead.
     *
     *                             0 - If IC_RESTART_EN is 1, a RESTART is
     * issued only if the transfer direction is changing from the previous
     ...
     *
     *                             2. In order to read the 11 bit, the user has
     * to perform the first data byte read [7:0] (offset 0x10) and then perform
     * the second read [15:8] (offset 0x11) in order to know the status of 11
     * bit (whether the data received in previous read is a first data byte or
     * not).
     *
     *                             3. The 11th bit is an optional read field,
     * user can ignore 2nd byte read [15:8] (offset 0x11) if not interested in
     ...
     * in 6 being set. For designs with APB_DATA_WIDTH = 8, the order of
     ...
     * to a value higher than 65525, because DW_apb_i2c uses a 16-bit counter
     * to flag an I2C bus idle condition when this counter reaches a value of
     * IC_SS_SCL_HCNT + 10.
     */
     ...
     * programming is important to ensure the correct operation of DW_apb_i2c.
     ...
     * (read-only) I2C Interrupt Status Register
     *
     *                     Each bit in this register has a corresponding mask
     * bit in the IC_INTR_MASK register. These bits are cleared by reading the
     ...
     *
     *                     These bits mask their corresponding interrupt status
     * bits. This register is active low; a value of 0 masks the interrupt,
     ...
     * the TX_FIFO and RX_FIFO whenever there is a transmit abort caused by any
     * of the events tracked by the IC_TX_ABRT_SOURCE register. The FIFOs
     ...
     *
     *                             Note: However, in high-speed mode or during
     * a START BYTE transfer, the RESTART comes before the address field as per
     * the I2C protocol. In this case, the slave is not the addressed slave
     * when the RESTART is issued, therefore DW_apb_i2c does not generate the
     ...
     * masked so they always show the true status of the DW_apb_i2c.
     */
     ...
     * that triggers the RX_FULL interrupt (bit 2 in IC_RAW_INTR_STAT
     * register). The valid range is 0-255, with the additional restriction
     * that hardware does not allow this value to be set to a value larger than
     * the depth of the buffer. If an attempt is made to do that, the actual
     * value set will be the maximum depth of the buffer. A value of 0 sets the
     * threshold for 1 entry, and a value of 255 sets the threshold for 256
     * entries.
     */
     ...
     * that trigger the TX_EMPTY interrupt (bit 4 in IC_RAW_INTR_STAT
     * register). The valid range is 0-255, with the additional restriction
     * that it may not be set to value larger than the depth of the buffer. If
     * an attempt is made to do that, the actual value set will be the maximum
     * depth of the buffer. A value of 0 sets the threshold for 0 entries, and
     ...
     * occurs: - The TX FIFO and RX FIFO get flushed. - Status bits in the
     * IC_INTR_STAT register are still active until DW_apb_i2c goes into IDLE
     * state. If the module is transmitting, it stops as well as deletes the
     * contents of the transmit buffer after the current transfer is complete.
     * If the module is receiving, the DW_apb_i2c stops the current transfer at
     * the end of the current byte and does not acknowledge the transfer.
     *
     *                             In systems with asynchronous pclk and ic_clk
     * when IC_CLK_TYPE parameter set to asynchronous (1), there is a two
     * ic_clk delay when enabling or disabling the DW_apb_i2c. For a detailed
     ...
     *
     *                             For a detailed description on how to abort
     * I2C transfers, refer to 'Aborting I2C Transfers'.
     *
     *                             Reset value: 0x0
     */
     ...
     *
     *                     This is a read-only register used to indicate the
     * current transfer status and FIFO status. The status register may be read
     * at any time. None of the bits in this register request an interrupt.
     *
     *                     When the I2C is disabled by writing 0 in bit 0 of
     * the IC_ENABLE register: - Bits 1 and 2 are set to 1 - Bits 3 and 10 are
     ...
     *
     *                     The bits [23:16] of this register are used to extend
     * the SDA transition (if any) whenever SCL is HIGH in the receiver in
     * either master or slave mode.
     *
     *                     Writes to this register succeed only when
     * IC_ENABLE[0]=0.
     *
     *                     The values in this register are in units of ic_clk
     * period. The value programmed in IC_SDA_TX_HOLD must be greater than the
     * minimum hold time in each mode (one cycle in master mode, seven cycles
     * in slave mode) for the value to be implemented.
     *
     ...
     * of the TX_ABRT bit. Except for Bit 9, this register is cleared whenever
     * the IC_CLR_TX_ABRT register or the IC_CLR_INTR register is read. To
     * clear Bit 9, the source of the ABRT_SBYTE_NORSTRT must be fixed first;
     ...
     * interface operation. There is a separate bit for transmit and receive.
     ...
     *
     *                             When read as 1, DW_apb_i2c is deemed to have
     * forced a NACK during any part of an I2C transfer, irrespective of
     ...
     * NACK a transfer, and IC_ENABLE[0] has been set to 0, then this bit will
     * also be set to 1.
     *
     *                             When read as 0, DW_apb_i2c is deemed to have
     * been disabled when there is master activity, or when the I2C bus is
     * idle.
     *
     ...
     *
     *                             When read as 0, DW_apb_i2c is deemed to have
     * been disabled without being actively involved in the data phase of a
     ...
     * hardware status when the IC_ENABLE[0] register is set from 1 to 0; that
     * is, when DW_apb_i2c is disabled.
     *
     *                     If IC_ENABLE[0] has been set to 1, bits 2:1 are
     * forced to 0, and bit 0 is forced to 1.
     *
     ...
     * (read-only) Component Parameter Register 1
     *
     *                     Note This register is not implemented and therefore
     * reads as 0. If it was implemented it would be a constant read-only
     * register that contains encoded information about the component's
     * parameter settings. Fields shown below are the settings for those
     * parameters
     */
...
static_assert(ifgen_struct<i2c0>);

static volatile i2c0 *const I2C0 = reinterpret_cast<i2c0 *>(0x40090000);

static volatile i2c0 *const I2C1 = reinterpret_cast<i2c0 *>(0x40098000);
