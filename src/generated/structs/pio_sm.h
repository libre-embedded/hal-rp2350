...
#pragma once
#ifndef RP2350_STRUCTS_PIO_SM_H
#define RP2350_STRUCTS_PIO_SM_H

#include "../enums/SM_EXECCTRL_STATUS_N.h"
#include "../enums/SM_EXECCTRL_STATUS_SEL.h"
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 24;   /*!< pio_sm's identifier. */
    ...
    uint32_t INSTR;   /*!< (read-write) Read to see the instruction currently
                addressed by state machine 0's program counter   Write to execute
                an instruction immediately (including jumps) and then resume
                execution. */
     ...
     */
    inline SM_EXECCTRL_STATUS_N get_EXECCTRL_STATUS_N() volatile
    {
        return SM_EXECCTRL_STATUS_N(EXECCTRL & 0b11111u);
    }
     ...
     * have undefined behaviour.
     */
    inline void set_EXECCTRL_STATUS_N(SM_EXECCTRL_STATUS_N value) volatile
    {
    ...
    /**
     * Get EXECCTRL's STATUS_SEL field.
     *
     * Comparison used for the MOV x, STATUS instruction.
     */
    inline SM_EXECCTRL_STATUS_SEL get_EXECCTRL_STATUS_SEL() volatile
    {
        return SM_EXECCTRL_STATUS_SEL((EXECCTRL >> 5u) & 0b11u);
    }

    /**
     * Set EXECCTRL's STATUS_SEL field.
     *
     * Comparison used for the MOV x, STATUS instruction.
     */
    inline void set_EXECCTRL_STATUS_SEL(SM_EXECCTRL_STATUS_SEL value) volatile
    {
        uint32_t curr = EXECCTRL;
     ...
     */
    inline void get_EXECCTRL(SM_EXECCTRL_STATUS_N &STATUS_N,
                             SM_EXECCTRL_STATUS_SEL &STATUS_SEL,
                             ...
                             bool &EXEC_STALLED) volatile
    {
        ...
        STATUS_N = SM_EXECCTRL_STATUS_N(curr & 0b11111u);
        STATUS_SEL = SM_EXECCTRL_STATUS_SEL((curr >> 5u) & 0b11u);
     ...
     * (read-write) Execution/behavioural settings for state machine 0
     */
    inline void set_EXECCTRL(SM_EXECCTRL_STATUS_N STATUS_N,
                             SM_EXECCTRL_STATUS_SEL STATUS_SEL,
                             uint8_t WRAP_BOTTOM, uint8_t WRAP_TOP,
                             bool OUT_STICKY, bool INLINE_OUT_EN,
                             uint8_t OUT_EN_SEL, uint8_t JMP_PIN,
    ...
    /**
     * Get SHIFTCTRL's IN_COUNT field.
     *
     ...
     * masking pins.
     */
    inline uint8_t get_SHIFTCTRL_IN_COUNT() volatile
    {
        return SHIFTCTRL & 0b11111u;
    }

    /**
     * Set SHIFTCTRL's IN_COUNT field.
     *
     * Set the number of pins which are not masked to 0 when read by an IN
     * PINS, WAIT PIN or MOV x, PINS instruction.
     *
     ...
     * masking pins.
     */
    inline void set_SHIFTCTRL_IN_COUNT(uint8_t value) volatile
    {
    ...
    /**
     * Get SHIFTCTRL's FJOIN_RX_GET bit.
     *
     ...
     * FJOIN_RX bits.
     */
    inline bool get_SHIFTCTRL_FJOIN_RX_GET() volatile
    {
        return SHIFTCTRL & (1u << 14u);
    }

    /**
     * Set SHIFTCTRL's FJOIN_RX_GET bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random read access by the state machine (using the `get`
     * instruction) and, unless FJOIN_RX_PUT is also set, random write access
     ...
     * FJOIN_RX bits.
     */
    inline void set_SHIFTCTRL_FJOIN_RX_GET() volatile
    {
        SHIFTCTRL |= 1u << 14u;
    }

    /**
     * Clear SHIFTCTRL's FJOIN_RX_GET bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random read access by the state machine (using the `get`
     * instruction) and, unless FJOIN_RX_PUT is also set, random write access
     ...
     * FJOIN_RX bits.
     */
    inline void clear_SHIFTCTRL_FJOIN_RX_GET() volatile
    {
        SHIFTCTRL &= ~(1u << 14u);
    }

    /**
     * Toggle SHIFTCTRL's FJOIN_RX_GET bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random read access by the state machine (using the `get`
     * instruction) and, unless FJOIN_RX_PUT is also set, random write access
     ...
     * FJOIN_RX bits.
     */
    inline void toggle_SHIFTCTRL_FJOIN_RX_GET() volatile
    {
        SHIFTCTRL ^= 1u << 14u;
    }

    /**
     * Get SHIFTCTRL's FJOIN_RX_PUT bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     ...
     * FJOIN_RX bits.
     */
    inline bool get_SHIFTCTRL_FJOIN_RX_PUT() volatile
    {
        return SHIFTCTRL & (1u << 15u);
    }

    /**
     * Set SHIFTCTRL's FJOIN_RX_PUT bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random write access by the state machine (using the `put`
     * instruction) and, unless FJOIN_RX_GET is also set, random read access by
     ...
     * FJOIN_RX bits.
     */
    inline void set_SHIFTCTRL_FJOIN_RX_PUT() volatile
    {
        SHIFTCTRL |= 1u << 15u;
    }

    /**
     * Clear SHIFTCTRL's FJOIN_RX_PUT bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random write access by the state machine (using the `put`
     * instruction) and, unless FJOIN_RX_GET is also set, random read access by
     ...
     * FJOIN_RX bits.
     */
    inline void clear_SHIFTCTRL_FJOIN_RX_PUT() volatile
    {
        SHIFTCTRL &= ~(1u << 15u);
    }

    /**
     * Toggle SHIFTCTRL's FJOIN_RX_PUT bit.
     *
     * If 1, disable this state machine's RX FIFO, make its storage available
     * for random write access by the state machine (using the `put`
     * instruction) and, unless FJOIN_RX_GET is also set, random read access by
     ...
     * FJOIN_RX bits.
     */
    inline void toggle_SHIFTCTRL_FJOIN_RX_PUT() volatile
    {
        SHIFTCTRL ^= 1u << 15u;
    }
     ...
     * state machine 0
     */
    inline void get_SHIFTCTRL(uint8_t &IN_COUNT, bool &FJOIN_RX_GET,
     ...
     * state machine 0
     */
    inline void set_SHIFTCTRL(uint8_t IN_COUNT, bool FJOIN_RX_GET,
                              bool FJOIN_RX_PUT, bool AUTOPUSH, bool AUTOPULL,
